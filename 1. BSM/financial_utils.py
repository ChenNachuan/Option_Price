...
def estimate_r_from_put_call_parity(
    option_data: List[Dict[str, Any]], 
    S: float, 
    T: float
) -> Optional[float]:
    """
    使用看涨-看跌平价的线性回归，从一组期权中稳健地估算 r。
    
    参数:
    option_data: 一个字典列表，包含 [{'K': 3800, 'C': 50.0, 'P': 40.0}, ...]
    S: 当前的现货价格估算值
    T: 该组期权的到期时间 (年化)
    
    返回:
    估算的无风险利率 r，如果数据不足则返回 None
    """
    if not option_data or T < EPSILON:
        return None
    ...
    for item in option_data:
        ...
        c_minus_p_values.append(item['C'] - item['P'])
        
    # 需要至少 N 个点（例如3个）来进行有意义的回归
    if len(k_values) < 3:
        return None

    try:
        # 核心：执行 Y = A + B*X 的线性回归
        # Y = c_minus_p_values, X = k_values
        # B 是斜率 (coeffs[0]), A 是截距 (coeffs[1])
        coeffs = np.polyfit(k_values, c_minus_p_values, 1)
        slope_B = coeffs[0]
        ...
        r_est = -np.log(-slope_B) / T
        
        # 避免极端值 (例如 r > 50% 或 r < 0%)
        if not np.isfinite(r_est) or r_est < 0 or r_est > 0.5:
            return None
