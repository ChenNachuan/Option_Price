...
# --- 3. 全局常量和配置 ---
RISK_FREE_RATE = 0.025  # 假设固定无风险利率为 2.5%
...
def main():
    ...
    # 预测价格的目标列表 (与官方 demo 中的 target_op 作用相同)
    ...
    # --- 4.4 主循环 ---
    # 这个循环由 iter_test() 驱动，每 500ms 运行一次
    for future_md, option_md, predict in iter_test:
        ...
        try:
            ...
            # 只有在 option_md 不是 None 时 (即每10分钟)，才执行这个昂贵的 "学习" 步骤
            
            if option_md:
                # print(f"[{current_time_str}] 收到期权行情，正在重新拟合波动率曲面...")
            ...
            # 无论 option_md 是否存在，这一步都会执行
            # 这替换了 Demo 中 `target_op[i] = mid_p` 的逻辑
            ...
            # 捕获循环中的任何意外错误，以防止程序崩溃
            print(f"主循环出错: {e}")
            # 尝试提交上一次的有效价格，以保持程序运行
            try:
                predict.target = target_prices 
                api.predict(predict)
            except:
